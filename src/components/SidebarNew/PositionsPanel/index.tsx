...
const PositionsPanel = (props: IPositionsPanel) => {
  ...
  const renderPositions = (open: boolean) => {
    ...
    return props.trades.closed.map((position: IClosedTrade, index: number) => {
      return (
        <ClosedPositionItem
          isOpen={
            props.selectedTrade &&
          ...
          colors={props.colors}
          key={index}
          selected={Number(position.instrumentID) === props.selectedInstrument}
          ...
          <div className="positions-actions">
            ...
            {props.canLoadMore && (
              <div
                ...
                onClick={props.actionLoadMoreClosedTradesRequest}
              >{t`load more`}</div>
            )}
          </div>
        )}
